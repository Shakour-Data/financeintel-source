...
import { calculateCryptoScore } from '../src/lib/scoring-engine-v2';
...
async function main() {
  ...
  const currentVersion = (latestCoeff?.version ?? 0) + 1;
    ...
    for (const { date } of chunkDates) {
      ...
      for (const rmd of rawMarketData) {
        try {
          ...
          console.error(`Score error for ${rmd.coin.coingeckoId} on ${date}:`, msg);
        }
      }
    }
